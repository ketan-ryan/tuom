...
public class LibEntityAILlamaFollowCaravan extends EntityAIBase {
    ...
    public LibEntityLlama llama;
    private double speedModifier;
    private int distCheckCounter;

    public LibEntityAILlamaFollowCaravan(LibEntityLlama llamaIn, double speedModifierIn) {
        this.llama = llamaIn;
    ...
    public boolean shouldExecute() {
        if (!this.llama.getLeashed() && !this.llama.inCaravan()) {
            List<LibEntityLlama> list = this.llama.world.<LibEntityLlama>getEntitiesWithinAABB(this.llama.getClass(), this.llama.getEntityBoundingBox().grow(9.0D, 4.0D, 9.0D));
            LibEntityLlama entityllama = null;
            double d0 = Double.MAX_VALUE;

            for (LibEntityLlama entityllama1 : list) {
            ...
            if (entityllama == null) {
                for (LibEntityLlama entityllama2 : list) {
    ...
    private boolean firstIsLeashed(LibEntityLlama p_190858_1_, int p_190858_2_) {
        if (p_190858_2_ > 8) {
            return false;
            ...
            if (p_190858_1_.getCaravanHead().getLeashed()) {
                ...
                LibEntityLlama entityllama = p_190858_1_.getCaravanHead();
                ++p_190858_2_;
