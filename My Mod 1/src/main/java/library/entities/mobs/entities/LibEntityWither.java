...
public class LibEntityWither extends LibEntityMob<LibEntityWither> implements IRangedAttackMob {

    /** COMMON VARIABLES **/

    // Sounds
    protected SoundEvent ambientSound = SoundEvents.ENTITY_WITHER_AMBIENT;
    protected SoundEvent hurtSound = SoundEvents.ENTITY_WITHER_HURT;
    protected SoundEvent deathSound = SoundEvents.ENTITY_WITHER_DEATH;
    ...
    protected void applyEntityAttributes() {
        ...
        double maxHealth = 300.0D; // max health
        double movementSpeed = 0.6D; // how fast it moves normally
        double followRange = 40.0D; // how many blocks away it will follow its target
        double armor = 4.0D; // multiplier for its health
    ...
    protected void initEntityAI() {
        this.tasks.addTask(0, new LibEntityWither.AIDoNothing());
        ...
        this.targetTasks.addTask(2, new EntityAINearestAttackableTarget(this, EntityLiving.class, 0, false, false, LibEntityWither.NOT_UNDEAD));
    }
    ...
    private static final DataParameter<Integer> FIRST_HEAD_TARGET = EntityDataManager.<Integer>createKey(LibEntityWither.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> SECOND_HEAD_TARGET = EntityDataManager.<Integer>createKey(LibEntityWither.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> THIRD_HEAD_TARGET = EntityDataManager.<Integer>createKey(LibEntityWither.class, DataSerializers.VARINT);
    ...
    private static final DataParameter<Integer> INVULNERABILITY_TIME = EntityDataManager.<Integer>createKey(LibEntityWither.class, DataSerializers.VARINT);
     ...
     */
    public static final Predicate<Entity> NOT_UNDEAD = new Predicate<Entity>() {
        @Override
    ...
    public LibEntityWither(World worldIn) {
        super(worldIn);
    ...
    public static void registerFixesWither(DataFixer fixer) {
        EntityLiving.registerFixesMob(fixer, LibEntityWither.class);
    }
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        if (this.isEntityInvulnerable(source)) {
            return false;
        } else if (source != DamageSource.DROWN && !(source.getTrueSource() instanceof LibEntityWither)) {
    ...
    public class AIDoNothing extends EntityAIBase {
        ...
        public boolean shouldExecute() {
            return LibEntityWither.this.getInvulTime() > 0;
        }
    }
}