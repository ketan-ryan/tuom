...
public class LibModelParrot extends LibModelBase {
    ModelRenderer body;
    ModelRenderer tail;
    ...
    private LibModelParrot.State state = LibModelParrot.State.STANDING;

    public LibModelParrot() {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
        ...
        float f = 0;//(float)Math.sin(ageInTicks * 0.3F);
        ageInTicks = (float)Math.sin(Math.toRadians(ageInTicks * 40F)) + 0.9F;
        ...
        if (this.state != LibModelParrot.State.FLYING) {
            if (this.state == LibModelParrot.State.SITTING) {
                return;
            }

            if (this.state == LibModelParrot.State.PARTY) {
    ...
    public void setLivingAnimations(EntityLivingBase entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTickTime) {
        ...
        if (entitylivingbaseIn instanceof EntityParrot) {
            ...
            if (entityparrot.isPartying()) {
                ...
                this.state = LibModelParrot.State.PARTY;
                return;
            }
